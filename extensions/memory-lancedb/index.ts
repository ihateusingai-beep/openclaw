...
type MemoryEntry = {
  ...
  createdAt: number;
  agentId?: string; // Per-agent isolation: prevents subagent memory overlap
};
...
function buildTableName(agentId?: string): string {
  // Per-agent isolation: each agent gets its own LanceDB table
  return agentId ? `memories_${agentId.replace(/[^a-zA-Z0-9_-]/g, "_")}` : "memories";
}
...
class MemoryDB {
  ...
  private async doInitialize(): Promise<void> {
    const tableName = buildTableName(this.agentId);
  ...
  async store(entry: Omit<MemoryEntry, "id" | "createdAt">): Promise<MemoryEntry> {
    ...
    const fullEntry: MemoryEntry = {
      ...
      id: randomUUID(),
      createdAt: Date.now(),
      agentId: this.agentId, // Tag with agentId for per-agent isolation
    };
  ...
  async search(vector: number[], limit = 5, minScore = 0.5): Promise<MemorySearchResult[]> {
    ...
    // Per-agent isolation: filter by agentId
    const agentIdFilter = this.agentId
      ? `agentId = "${this.agentId.replace(/"/g, '\\"')}"`
      : "agentId IS NULL";
